...
export default function CreateMarketPage() {
  ...
  useEffect(() => {
    const hideUIValue = searchParams.get('hideUI')
    const question = searchParams.get('question')

    // Auto-show wizard if hideUI is true or if any market data is provided
    if (hideUIValue === 'true' || question) {
      setHideUI(true)
      setShowWizard(true)
    }
  }, [searchParams])
    ...
    <div className="max-w-5xl mx-auto space-y-8 px-4">
      ...
      <div className="text-center space-y-4">
        ...
        <h1 className="text-4xl md:text-5xl font-bold">
          Turn Any Poll Into a{' '}
          <span className="gold-text">Prediction Market</span>
        </h1>
        <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
          Paste a Twitter or Farcaster poll URL and create a prediction market
          in minutes. Earn fees on every bet.
        </p>
        <Button
          size="lg"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-6 bg-card border-border card-glow">
          ...
          <p className="text-muted-foreground">
            Just paste a poll URL and configure basic settings. Your market goes
            live in minutes.
          </p>
        </Card>
        ...
        <Card className="p-6 bg-card border-border card-glow">
          ...
          <p className="text-muted-foreground">
            Set a creator fee (1-5%) and earn passive income on every bet placed
            in your market.
          </p>
        </Card>
        ...
        <Card className="p-6 bg-card border-border card-glow">
          ...
          <p className="text-muted-foreground">
            You decide the resolution criteria and resolve the market when the
            outcome is clear.
          </p>
        </Card>
      </div>
      ...
      <Card className="p-8 bg-gradient-to-br from-card to-bg-4 border-border text-center">
        ...
        <p className="text-muted-foreground mb-6">
          Join hundreds of creators earning from prediction markets
        </p>
        <Button
          size="lg"
