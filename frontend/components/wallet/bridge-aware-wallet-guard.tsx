...
export function BridgeAwareWalletGuard({ 
  ...
  useEffect(() => {
    ...
    const checkBridgeState = () => {
      try {
        const bridgeInProgress = sessionStorage.getItem('pop-bridge-in-progress');
        const bridgeOperationId = sessionStorage.getItem('pop-bridge-operation-id');
        
        if (bridgeInProgress === 'true' && bridgeOperationId) {
          setIsBridgeInProgress(true);
          console.log('Bridge operation detected, preventing wallet guard redirect');
        } else {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'pop-bridge-in-progress' || e.key === 'pop-bridge-operation-id') {
        checkBridgeState();
      }
    };
  ...
  useEffect(() => {
    ...
    setHasChecked(true);
  }, [mounted, isConnected, isCorrectChain, isConnecting, isSwitchingChain, requireCorrectChain, hasChecked, isBridgeInProgress]);
  ...
  if (isBridgeInProgress) {
    console.log('Bridge operation in progress, allowing access to prevent redirect');
    return <>{children}</>;
  }
